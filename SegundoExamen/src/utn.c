...
/**
 * \brief Le pide al usuario un número entero
 ...
 * \return Retorna 0 (EXITO) si se obtiene un número o -1 (ERROR) si no*/
int utn_getNumero(int* pNumero, char* pMensaje, char* pMensajeError, int min, int max)
{
	int retorno = -1;
	int bufferInt;

	if(pNumero != NULL && pMensaje != NULL && pMensajeError != NULL && min <= max)
	{
		printf("%s", pMensaje);
		__fpurge(stdin);
...
int utn_getNumeroConIntentos(int* pNumero, char* pMensaje, char* pMensajeError, int min, int max, int reintentos)
{
	int retorno = -1;
	int bufferInt;
...
/**
 * \brief Le pide al usuario un número decimal
...
int utn_getDecimal(float* pDecimal, char* pMensaje, char* pMensajeError, float min, float max)
{
	int retorno = -1;
	float bufferFloat;

	if(pDecimal != NULL && pMensaje != NULL && pMensajeError != NULL && min <= max)
	{
		printf("%s", pMensaje);
		__fpurge(stdin);
...
int utn_getDecimalConIntentos(float* pDecimal, char* pMensaje, char* pMensajeError, float min, float max, int reintentos)
{
	int retorno = -1;
	float bufferFloat;
